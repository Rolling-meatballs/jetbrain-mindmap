...
3. 在 kityminder-editor 根目录下运行 `npm run dev`（由 Vite 驱动）即可启动项目
...
可参考根目录下 `vite.config.cjs` 和 `scripts/build-core.js`。

## 构建
运行 `npm run build`，完成后 `dist` 目录里就是可用运行的 kityminder-editor, 双击 `index.html` 即可打开运行示例

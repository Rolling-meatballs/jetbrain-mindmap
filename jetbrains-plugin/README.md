...
> Build system now uses **IntelliJ Platform Gradle Plugin 2.x** and targets **IntelliJ Platform 261+ IDEs**.

> Marketplace update notifications only reach installed IDEs when the published plugin version is compatible with that IDE build. Version `0.1.8` keeps `sinceBuild=261` and removes the upper build cap so 2026.1 and 2026.2 IDEs can receive compatible updates.
